...
parking_buffer_temp = [None]*len(parking_data)
...
while True:
    ...
    if config['parking_detection']:        
        for ind, park in enumerate(parking_data):
            ...
            if parking_buffer_temp[ind] != parking_status[ind]: 
                try:
                    #data = "slot-3"
                ...
                try:
                    #status = True
                    doc_ref.update({u'Status': str(status)})
                    #print("sukses")
                except:
                    print ("Koneksi Gagal")
                parking_buffer_temp[ind] = parking_status[ind]
